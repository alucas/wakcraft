...
public class PlayerEventHandler {
    ...
    public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (!event.player.worldObj.isRemote && event.player instanceof EntityPlayerMP) {
            ...
            Wakcraft.packetPipeline.sendTo(new PacketExtendedEntityProperty(event.player, QuestInstanceProperty.IDENTIFIER), playerMP);
            ...
            if (properties.isInHavenBag()) {
                Wakcraft.packetPipeline.sendTo(new PacketHavenBagProperties(properties.getCurrentHavenBagId()), playerMP);
            }
        }
    }
